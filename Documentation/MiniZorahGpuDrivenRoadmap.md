...
**当前进度：M1、M2、M3 已完成；M4 已接入同帧准入、自适应预取及逐页延迟统计，三个固定视角保持 1.5 px 度量收敛。** 无预取控制组隔离的准入改动使请求 P95 从 132 ms 降到 114 ms；开启预取后缺页请求减少约 60%，25 秒视角切换的恢复从约 1.25 秒缩短到 0.26 秒，额外上传约 16.3%。剩余冷启动缺页 P95 为 169 ms，不能宣称全部请求尾部已改善。固定近景图像与上轮逐像素一致，41 项相关回归通过。见 [最新预取与尾延迟验收](MiniZorahPrefetch.md)、[质量收敛](MiniZorahQuality.md)、[成本与收益调度](MiniZorahScreenBenefit.md)、[遍历与页面复用](MiniZorahRoamingOptimization.md)、[M4 初始基线](MiniZorahRoaming.md)、[M3 统一 VBuffer](MiniZorahVBuffer.md)。
...
已完成实际候选派发、视锥前移、并行 prefix、协作 frontier/emit、CPU 批量退避、有界 I/O、驻留复用、并行硬件 mesh 输出、late 重试 mask、收益排序和层级需求可见性。本轮新增逐页延迟度量、同帧准入及受 CPU 剩余额度约束的 GPU 预取。[最新结果](MiniZorahPrefetch.md)；接下来针对冷启动请求批次、发布等待与 early 分类成本。
...
| PR 9 | 冷启动批次与发布等待、early 分类和候选准备成本 | 分离缓存命中与尚未就绪的预取需求，基于实际队列完成证据减少等待，继续约束质量、工作集和吞吐 |

M1–M3 的完整 cook、独立首帧和统一 VBuffer 已落地。M4 已接入屏幕收益排序、层级需求裁剪与自适应预取，固定视角达到 cook 误差度量下的 1.5 px 收敛。下一步针对剩余冷启动请求尾部与 early 分类成本；64 MiB 压力回退仍不算质量验收，检查点结果也不等同于每个运动帧的质量保证。
