...
**当前进度：M1、M2、M3 已完成；M4 已达到三个固定视角的 1.5 px 度量收敛，并通过 180 秒三轮漫游；首次加载与转向瞬态仍需优化。** 新增包含细化后代的需求范围裁剪，近景有效驻留从 1,023 MiB 降至 238 MiB，收敛图像逐像素一致。三轮漫游 GPU P95 为 8.75 ms、同步离屏帧 P95 为 12.41 ms；65～175 秒的 23 个检查点无可见超标 refinement。这些指标不是编辑器呈现 FPS。见 [最新质量收敛验收](MiniZorahQuality.md)、[上一轮成本与收益调度](MiniZorahScreenBenefit.md)、[遍历与页面复用](MiniZorahRoamingOptimization.md)、[M4 初始基线](MiniZorahRoaming.md)、[M3 统一 VBuffer](MiniZorahVBuffer.md)。
...
已完成实际候选派发、视锥前移、并行 prefix、协作 frontier/emit、CPU 批量退避、有界 I/O、驻留复用、并行硬件 mesh 输出、late 重试 mask 和收益排序。本轮完成层级需求可见性与固定视角质量验收。[最新结果](MiniZorahQuality.md)；接下来推进逐页延迟度量和首次视角切换瞬态。
...
M2 的 CPU 重复淘汰扫描和碎片空闲列表重复失败扫描已修复。GPU 未使用页留在预算内缓存，仅在压力下回收已确认的冷页，根页和近期需求得到保护。本轮 1 GiB 路线在 55 秒检查点的累计上传从 3.99 GB 降至 0.60 GB、驱逐从 74,740 页降至 0；三轮后工作集约 578 MiB。64 MiB 档仍只满足完整回退，不满足 1.5 px 质量。
...
| 紧凑编码与流式优先级 | 浮点 payload、CPU 解压；已接入屏幕收益排序及层级需求裁剪 | 三轮工作集可容纳于 1 GiB，优先测切换尾延迟，再决定压缩投入 |
...
| PR 8 | 首次需求到 drawable 尾延迟、运动预取/视口边缘滞后与 early 分类成本 | 在同一质量和预算条件下减少首轮视角切换瞬态，控制新增上传与驻留 |

M1–M3 的完整 cook、独立首帧和统一 VBuffer 已落地。M4 已接入屏幕收益排序与层级需求裁剪，固定视角达到 cook 误差度量下的 1.5 px 收敛，三轮路径工作集可容纳于 1 GiB。下一步针对首轮运动的需求尾延迟与 early 分类成本；64 MiB 压力回退仍不算质量验收，检查点结果也不等同于每个运动帧的质量保证。
