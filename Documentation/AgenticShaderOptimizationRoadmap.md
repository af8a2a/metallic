...
[M3 单候选闭环](AgenticShaderOptimizationM3.md)已实现受保护 patch/build、正确性检查、ABBA、独立确认门槛、机器判定、归档及基线恢复。MiniZorah 首个真实候选完成 12 进程 / 6 对 A/B，输出一致、监控完整，但软件耗时增加 11.48%，自动 reject。前两批监控缺口保留为 inconclusive；未接受或安装候选。Full 仍受 M2 驻留差异限制，独立确认分支目前由合成完整证据测试覆盖，实机候选因初测拒绝未进入该分支。
