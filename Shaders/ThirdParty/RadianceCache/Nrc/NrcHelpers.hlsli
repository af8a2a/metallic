 ...
 *
 * Ported from the NRC SDK (Include/NrcHelpers.hlsli) for Metallic's Slang/SPIR-V
 * path tracer; packing helpers are unchanged.
 */

#ifndef __METALLIC_NRC_HELPERS_HLSL__
#define __METALLIC_NRC_HELPERS_HLSL__
...
*/
uint NrcPackSnorm2x16(float2 v)
{
...
float2 NrcUnpackSnorm2x16(uint packed)
{
    ...
    float2 unpacked = max((float2)bits / 32767.f, -1.0f);
    return unpacked;
}
...
*/
float3 NrcOctToDirection(float2 p)
{
...
*/
uint NrcIncrementCounter(RWStructuredBuffer<uint> countersData, NrcCounter counter)
{
    //< Optimized version with one InterlockedAdd per warp. This can be always on
...
#endif // __METALLIC_NRC_HELPERS_HLSL__
