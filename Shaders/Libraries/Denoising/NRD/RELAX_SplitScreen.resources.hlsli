...
NRD_INPUTS_START
    NRD_INPUT( Texture2D, float, gIn_ViewZ, 0 )
        ...
        #if( NRD_MODE == SH )
            NRD_INPUT( Texture2D, float4, gIn_DiffSh, 3 )
            NRD_INPUT( Texture2D, float4, gIn_SpecSh, 4 )
        #endif
    #elif( NRD_DIFF )
        NRD_INPUT( Texture2D, float4, gIn_Diff, 1 )
        #if( NRD_MODE == SH )
            NRD_INPUT( Texture2D, float4, gIn_DiffSh, 2 )
        #endif
    #else
        NRD_INPUT( Texture2D, float4, gIn_Spec, 1 )
        #if( NRD_MODE == SH )
            NRD_INPUT( Texture2D, float4, gIn_SpecSh, 2 )
        #endif
    #endif
NRD_INPUTS_END
...
NRD_OUTPUTS_START
        ...
        #if( NRD_MODE == SH )
            NRD_OUTPUT( RWTexture2D, float4, gOut_DiffSh, 2 )
