...
NRD_INPUTS_START
    ...
    NRD_INPUT( Texture2D, float, gIn_ViewZ, 2 )
        ...
        NRD_INPUT( Texture2D, REBLUR_TYPE, gIn_Diff, 3 )
        NRD_INPUT( Texture2D, REBLUR_TYPE, gIn_Spec, 4 )
        #if( NRD_MODE == SH )
            NRD_INPUT( Texture2D, REBLUR_SH_TYPE, gIn_DiffSh, 5 )
            NRD_INPUT( Texture2D, REBLUR_SH_TYPE, gIn_SpecSh, 6 )
        #endif
    #elif( NRD_DIFF )
        NRD_INPUT( Texture2D, REBLUR_TYPE, gIn_Diff, 3 )
        #if( NRD_MODE == SH )
            NRD_INPUT( Texture2D, REBLUR_SH_TYPE, gIn_DiffSh, 4 )
        #endif
    #else
        NRD_INPUT( Texture2D, REBLUR_TYPE, gIn_Spec, 3 )
        #if( NRD_MODE == SH )
...
NRD_OUTPUTS_START
        ...
        NRD_OUTPUT( RWTexture2D, float, gOut_SpecHitDistForTracking, 2 )
        #if( NRD_MODE == SH )
        ...
        NRD_OUTPUT( RWTexture2D, float, gOut_SpecHitDistForTracking, 1 )
        #if( NRD_MODE == SH )
